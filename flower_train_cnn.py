...
DATA_DIR = "/home/qianxinming/tensorflow_input_image_by_tfrecord-master_3/src"
...
def flower_train():
    ...
    label_batch_placeholder = tf.placeholder(tf.float32, shape=[BATCH_SIZE, 2])
    label_offset = -tf.ones([BATCH_SIZE], dtype=tf.int64, name="label_batch_offset")
    label_batch_one_hot = tf.one_hot(tf.add(label_batch_out, label_offset), depth=2, on_value=1.0, off_value=0.0)
...
#    loss = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=label_batch_one_hot, logits=logits_out))
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "/home/qianxinming/tensorflow_input_image_by_tfrecord-master_3/src/saver/checkpoint-train.ckpt")
        ...
        for i in range(TRAINING_SET_SIZE * 100):
            ...
            print(loss_out)
            if i % 50 == 0:
                saver.save(sess, "/home/qianxinming/tensorflow_input_image_by_tfrecord-master_3/src/saver/checkpoint-train.ckpt")
